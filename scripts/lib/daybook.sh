#!/usr/bin/env bash
# Typed event log for /iterate cycles (the "daybook").
...
#
# Usage:
#   source scripts/lib/daybook.sh
#   daybook_event <log_path> <kind> <phase> <agent> <payload_json>
#
...
daybook_event() {
  ...
  if [ -z "$log_path" ]; then
    echo "daybook_event: log_path required" >&2
    return 1
  fi
  if [ -z "$kind" ]; then
    echo "daybook_event: kind required" >&2
    return 1
  fi
  if [ -z "$phase" ]; then
    echo "daybook_event: phase required" >&2
    return 1
  fi

  # Validate kind against closed enum before spawning python.
  local known=0 k
  for k in "${DAYBOOK_KINDS[@]}"; do
    [ "$k" = "$kind" ] && known=1 && break
  done
  if [ "$known" -ne 1 ]; then
    echo "daybook_event: unknown kind '$kind'" >&2
    return 1
  fi

  # cycle_id comes from parent directory name by convention — keeps the
  # daybook self-describing even when paths move.
  local cycle_dir cycle_id
  cycle_dir="$(dirname "$log_path")"
  cycle_id="$(basename "$cycle_dir")"

  DAYBOOK_LOG="$log_path" \
  DAYBOOK_KIND="$kind" \
  DAYBOOK_PHASE="$phase" \
  DAYBOOK_AGENT="$agent" \
  DAYBOOK_CYCLE_ID="$cycle_id" \
  DAYBOOK_PAYLOAD="$payload" \
  python3 <<'PYEOF'
import fcntl, json, os, sys, time

log_path = os.environ["DAYBOOK_LOG"]
payload_raw = os.environ.get("DAYBOOK_PAYLOAD", "{}") or "{}"

try:
    payload = json.loads(payload_raw)
except Exception as e:
    print(f"daybook_event: invalid payload JSON: {e}", file=sys.stderr)
    sys.exit(1)
if not isinstance(payload, dict):
    print("daybook_event: payload must be a JSON object", file=sys.stderr)
    sys.exit(1)

event = {
    "schema_version": 1,
    "ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    "cycle_id": os.environ["DAYBOOK_CYCLE_ID"],
    "kind":  os.environ["DAYBOOK_KIND"],
    "phase": os.environ["DAYBOOK_PHASE"],
    "agent": os.environ["DAYBOOK_AGENT"],
}
