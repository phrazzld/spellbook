...
name: groom
description: |
  Interactive backlog refinement, backlog doctrine, and automated cleanup.
  ...
  Use when: backlog session, issue refinement, sprint planning, backlog cleanup,
  backlog overhaul, rewriting tickets, reducing issue sprawl, agent-executable issues.
  Trigger: /groom, /refine (alias), /backlog, /tidy, "groom the backlog",
  "clean up issues", "overhaul the backlog", "rewrite tickets", "agent-ready issues".
---
...
2. **Codebase Health Audit** — run assess-* suite against the full codebase
   (strong tier agent) to discover quality debt:
   - `assess-depth` (full-repo, not just diff) — shallow modules, Ousterhout violations
   - `assess-drift` (full architecture review) — boundary violations, naming drift
   - `assess-tests` (test suite health) — implementation coupling, mock depth
   - `assess-docs` (documentation health) — stale references, outdated examples

   Synthesize findings into tech-debt issues with intent contracts. Strong agent
   reviews all assess-* outputs, prioritizes by impact and effort, creates
   GitHub issues, feeds into backlog ordering. This phase means `/groom` doesn't
   just look at existing issues — it actively discovers new quality debt.

2.1. **Architecture Critique** — four parallel tracks:
